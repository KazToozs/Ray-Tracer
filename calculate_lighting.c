...
** Last update Wed Mar 18 17:22:16 2015 cristopher toozs-hobson
*/
...
{
  // set les coordonnées du point d'intersection vecteur(camera)/objet
  s->x.c.p.x = s->c.p.x + (s->x.k * s->c.v.vx);
  s->x.c.p.y = s->c.p.y + (s->x.k * s->c.v.vy);
  s->x.c.p.z = s->c.p.z + (s->x.k * s->c.v.vz);
  // set les coordonnées du vecteur directeur intersection->lumière
...
void		light(t_st *s)
{
  ...
  //calculer les ombres direct après
  if (shadow(s) == -1)
    return ;
  //calculs lumière
  calculate_n(s);
